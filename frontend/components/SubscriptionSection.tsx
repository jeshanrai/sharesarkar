...
export default function SubscriptionSection() {
  const [insightsEmail, setInsightsEmail] = useState<FormState>({ value: "", submitted: false, loading: false });
  const [whatsappNumber, setWhatsappNumber] = useState<FormState>({ value: "", submitted: false, loading: false });
  const [signalsEmail, setSignalsEmail] = useState<FormState>({ value: "", submitted: false, loading: false });
  ...
  const handleSubmit = (
    ...
    setTimeout(() => {
      setter({ value: "", submitted: true, loading: false });
    }, 1000);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-gray-900 rounded-lg p-5">
          <div className="flex items-start justify-between mb-4">
            ...
            <span className="px-2 py-0.5 bg-white/10 text-white/70 text-[10px] font-medium rounded">
              Weekly
            </span>
          </div>
          ...
          {insightsEmail.submitted ? (
            <div className="flex items-center justify-center gap-2 py-2.5 bg-white/10 rounded-lg">
              ...
              <span className="text-xs text-white">Subscribed!</span>
            </div>
          ) : (
            <form onSubmit={(e) => handleSubmit(e, setInsightsEmail, insightsEmail)}>
              <input
                type="email"
                placeholder="Enter your email"
                value={insightsEmail.value}
                onChange={(e) => setInsightsEmail({ ...insightsEmail, value: e.target.value })}
              ...
              <button
                type="submit"
                disabled={insightsEmail.loading}
              ...
              >
                {insightsEmail.loading ? "..." : "Subscribe Free"}
              </button>
            </form>
          )}
        </div>
        ...
        <div className="bg-brand-green rounded-lg p-5 text-white">
          ...
          {whatsappNumber.submitted ? (
            <div className="flex items-center justify-center gap-2 py-2.5 bg-white/20 rounded-lg">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
              <span className="text-xs">You&apos;ll receive alerts!</span>
            </div>
          ) : (
            <form onSubmit={(e) => handleSubmit(e, setWhatsappNumber, whatsappNumber)}>
              <div className="flex gap-2 mb-2">
                ...
                <input
                  ...
                  onChange={(e) => setWhatsappNumber({ ...whatsappNumber, value: e.target.value })}
              ...
              >
                {whatsappNumber.loading ? "..." : "Get IPO Alerts"}
              </button>
            </form>
          )}
        </div>
        ...
        <div className="bg-gray-900 rounded-lg p-5">
          <div className="flex items-start justify-between mb-4">
            ...
            <span className="px-2 py-0.5 bg-brand-red/20 text-brand-red text-[10px] font-medium rounded">
              Pro
            </span>
          </div>
          ...
          {signalsEmail.submitted ? (
            <div className="flex items-center justify-center gap-2 py-2.5 bg-white/10 rounded-lg">
              <svg className="w-4 h-4 text-brand-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
              <span className="text-xs text-white">Signals activated!</span>
            </div>
          ) : (
            <form onSubmit={(e) => handleSubmit(e, setSignalsEmail, signalsEmail)}>
              <input
                type="email"
                placeholder="Enter your email"
                value={signalsEmail.value}
                onChange={(e) => setSignalsEmail({ ...signalsEmail, value: e.target.value })}
                className="w-full px-3 py-2.5 rounded-lg bg-white/5 placeholder-gray-500 text-white text-sm border border-white/10 focus:outline-none focus:border-white/30 mb-2"
                required
              />
              <button
                type="submit"
                disabled={signalsEmail.loading}
                className="w-full py-2.5 bg-white text-gray-900 rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors disabled:opacity-70"
              >
                {signalsEmail.loading ? "..." : "Get Free Signals"}
              </button>
            </form>
          )}
        </div>
