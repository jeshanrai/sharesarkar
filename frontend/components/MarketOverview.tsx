      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7">
          <div className="flex items-center justify-between mb-3">
            <h3 className="eyebrow text-gray-900">Sub-Indices</h3>
            <span className="text-[10px] text-gray-400 uppercase tracking-wider">Close · Δ%</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 border-t border-gray-200">
            {sectors.length === 0 && Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="border-b border-gray-100 py-3 px-1 skeleton h-12" />
            ))}
            {sectors.map((s) => {
              ...
              return (
                <Link
                  ...
                  className="group flex items-center justify-between py-3 px-1 border-b border-gray-100 hover:bg-gray-50 transition-colors"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`block w-1 h-6 ${up ? "bg-emerald-500" : "bg-red-500"}`} />
                    <span className="text-sm text-gray-800 group-hover:text-[#d32027] transition-colors truncate">
                      {s.index_name}
                    </span>
                  </div>
        ...
        <div className="lg:col-span-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="eyebrow text-gray-900">Most Active</h3>
          ...
          <div className="border-t border-gray-200">
            <div className="grid grid-cols-12 gap-2 py-2 border-b border-gray-100 text-[10px] uppercase tracking-wider text-gray-500">
            ...
            {top.length === 0 && Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 py-3 border-b border-gray-100">
                <div className="col-span-12 h-4 skeleton" />
              </div>
            ))}
            {top.map((r, i) => {
